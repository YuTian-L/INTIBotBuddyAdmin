...
public class IntimaFbActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_intima_fb);

        String category = getString(R.string.button_academicINTI);
        setTitle(category.toUpperCase());

        ProgressBar progressBar = findViewById(R.id.progressBarIntimaFb);
        progressBar.setVisibility(ProgressBar.VISIBLE);

        listView = findViewById(R.id.intimaFbListView);
        getJSON();
    }
    ...
    private void getJSON() {
        class GetJSON extends AsyncTask<Void, Void, Object> {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    String link = "http://pkunite2.000webhostapp.com/getID25.php";
                    ...
                    Log.e("IntimaFbActivityError", e.getLocalizedMessage());
    ...
    private void loadIntoListView(String line) throws JSONException {
        ...
        String[] intimaFb = new String[jsonObject.length()];
        Map<String, String> details = new HashMap<>();

        for (int i=0; jsonObject.has(String.valueOf(i+1)); i++){
            intimaFb[i] = jsonObject.getString(String.valueOf(i+1));
