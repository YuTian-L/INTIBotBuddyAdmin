...
public class ContactsAcademicActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_contacts_academic);

        String category = getString(R.string.button_contacts);
        setTitle(category.toUpperCase());

        ProgressBar progressBar = findViewById(R.id.progressBarContactsAcademic);
        progressBar.setVisibility(ProgressBar.VISIBLE);

        listView = findViewById(R.id.contactsAcademicListView);
        getJSON();
    }
    ...
    private void getJSON() {
        class GetJSON extends AsyncTask<Void, Void, Object> {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    String link = "http://pkunite2.000webhostapp.com/getID52.php";
                    ...
                    Log.e("ContAcadActivityError", e.getLocalizedMessage());
    ...
    private void loadIntoListView(String line) throws JSONException {
        ...
        String[] contAcademic = new String[jsonObject.length()];
        Map<String, String> details = new HashMap<>();

        for (int i=0; jsonObject.has(String.valueOf(i+1)); i++){
            contAcademic[i] = jsonObject.getString(String.valueOf(i+1));
