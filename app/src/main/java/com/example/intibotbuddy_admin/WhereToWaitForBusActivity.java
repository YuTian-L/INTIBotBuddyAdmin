...
public class WhereToWaitForBusActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_where_to_wait_for_bus);

        String category = getString(R.string.button_transportationServices);
        setTitle(category.toUpperCase());

        ProgressBar progressBar = findViewById(R.id.progressBarWhereToWaitForBus);
        progressBar.setVisibility(ProgressBar.VISIBLE);

        listView = findViewById(R.id.whereToWaitForBusListView);
        getJSON();
    }
    ...
    private void getJSON() {
        class GetJSON extends AsyncTask<Void, Void, Object> {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    String link = "http://pkunite2.000webhostapp.com/getID33.php";
                    ...
                    Log.e("WaitForBusActivityError", e.getLocalizedMessage());
    ...
    private void loadIntoListView(String line) throws JSONException {
        ...
        String[] waitForBus = new String[jsonObject.length()];
        Map<String, String> details = new HashMap<>();

        for (int i=0; jsonObject.has(String.valueOf(i+1)); i++){
            waitForBus[i] = jsonObject.getString(String.valueOf(i+1));
