...
public class AcademicWebsitesResActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_academic_websites_res);

        String category = getString(R.string.button_academicINTI);
        setTitle(category.toUpperCase());

        ProgressBar progressBar = findViewById(R.id.progressBarAcademicWebsitesRes);
        progressBar.setVisibility(ProgressBar.VISIBLE);

        listView = findViewById(R.id.academicWebResListView);
        getJSON();
    }
    ...
    private void getJSON() {
        class GetJSON extends AsyncTask<Void, Void, Object> {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    String link = "http://pkunite2.000webhostapp.com/getID24.php";
                    ...
                    Log.e("AcaWebResActivityError", e.getLocalizedMessage());
    ...
    private void loadIntoListView(String line) throws JSONException {
        ...
        String[] academicWebRes = new String[jsonObject.length()];
        Map<String, String> details = new HashMap<>();

        for (int i=0; jsonObject.has(String.valueOf(i+1)); i++){
            academicWebRes[i] = jsonObject.getString(String.valueOf(i+1));
