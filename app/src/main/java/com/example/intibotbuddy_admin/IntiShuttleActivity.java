...
public class IntiShuttleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_inti_shuttle);

        String category = getString(R.string.button_transportationServices);
        setTitle(category.toUpperCase());

        ProgressBar progressBar = findViewById(R.id.progressBarIntiShuttle);
        progressBar.setVisibility(ProgressBar.VISIBLE);

        listView = findViewById(R.id.intiShuttleListView);
        getJSON();
    }
        ...
        class GetJSON extends AsyncTask<Void, Void, Object> {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    ...
                    Log.e("IntiShutActivityError", e.getLocalizedMessage());
    ...
    private void loadIntoListView(String line) throws JSONException {
        ...
        String[] intiShuttle = new String[jsonObject.length()];
        Map<String, String> details = new HashMap<>();

        for (int i=0; jsonObject.has(String.valueOf(i+1)); i++){
            intiShuttle[i] = jsonObject.getString(String.valueOf(i+1));
