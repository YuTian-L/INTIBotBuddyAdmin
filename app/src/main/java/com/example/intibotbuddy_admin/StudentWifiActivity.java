...
public class StudentWifiActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_student_wifi);

        String category = getString(R.string.button_wifiInternet);
        setTitle(category.toUpperCase());

        ProgressBar progressBar = findViewById(R.id.progressBarStudentWifi);
        progressBar.setVisibility(ProgressBar.VISIBLE);

        listView = findViewById(R.id.studentWifiListView);
        getJSON();
    }
    ...
    private void getJSON() {
        class GetJSON extends AsyncTask<Void, Void, Object> {
            ...
            protected Object doInBackground(Void... voids) {
                try {
                    String link = "http://pkunite2.000webhostapp.com/getID81.php";
                    ...
                    Log.e("StudWifiActivityError", e.getLocalizedMessage());
    ...
    private void loadIntoListView(String line) throws JSONException {
        ...
        String[] studentWifi = new String[jsonObject.length()];
        Map<String, String> details = new HashMap<>();

        for (int i=0; jsonObject.has(String.valueOf(i+1)); i++){
            studentWifi[i] = jsonObject.getString(String.valueOf(i+1));
